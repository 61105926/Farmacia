  ...
  <AdminLayout>
    <template #header>
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold text-gray-900">Nueva Farmacia</h1>
          <p class="text-gray-600 mt-1">Registra una nueva farmacia cliente</p>
        </div>
    ...
    <form @submit.prevent="submit" class="space-y-8">
      ...
      <Card>
        ...
        <CardContent class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <Label for="codigo_cliente">Código Cliente (Auto-generado)</Label>
              <Input
                id="codigo_cliente"
              ...
              />
              <p class="text-xs text-gray-500 mt-1">Se generará automáticamente al crear la farmacia</p>
            </div>
            <div>
      ...
      <div class="flex justify-end space-x-4">
        ...
        >
          ...
          {{ form.processing ? 'Guardando...' : 'Guardar Farmacia' }}
        </Button>
      </div>
    </form>
  </AdminLayout>
  ...
  form.post($route('pharmacies.store'), {
    onSuccess: () => {
      window.$notify?.success(
        'Farmacia creada',
        'La farmacia ha sido registrada exitosamente'
      )
    },
    onError: () => {
      window.$notify?.error(
